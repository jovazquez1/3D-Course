...
class FixedData:
	...
	var bullet_visual_does_get_deleted := false
...
func _setup_populate_test_space() -> void:
	...
	await _connect_timed(duration,
get_tree().physics_frame,
func collect_frame_data():
	...
	if practice_bullet.visual != null:
		practice_bullet.visual.tree_exiting.connect(func ():
			fixed_data.bullet_visual_does_get_deleted = true
		...
		practice_bullet.tree_exiting.connect(func ():
...
func _build_checks() -> void:
	...
	check_skin_disappears.checker = func () -> String:
		if fixed_data.scale_at_end == Vector3.ONE:
			return tr("The bullet's skin does not seem to play its disappear animation when the bullet is destroyed. Did you call the bullet skin's destroy() method? When calling it,
be sure to turn off physics processing with set_process(false) to prevent the bullet from moving after it's destroyed.")
		return ""
	...
	check_bullet_moves_forward.checker = func () -> String:
		var is_bullet_moving_forward := func (frame_previous: FrameData,
frame_current: FrameData) -> bool:
	...
	var check_bullet_visual_dies := Check.new()
	check_bullet_visual_dies.description = tr("The bullet gets destroyed upon reaching its maximum range.")
	check_bullet_visual_dies.checker = func () -> String:
		if not fixed_data.bullet_visual_does_get_deleted:
	...
	var check_bullet_dies_after_visual := Check.new()
	check_bullet_dies_after_visual.description = tr("The bullet is freed when the visuals are freed.")
	check_bullet_dies_after_visual.checker = func () -> String:
		if not fixed_data.bullet_does_get_deleted:
			return tr("The bullet was not freed. Did you connect the bullet visuals' tree_exited signal to the bullet's queue_free() method?")
		return ""
	check_bullet_dies_after_visual.dependencies = [check_bullet_visual_dies]

	checks = [
		check_skin_configurable,
		check_skin_instantiated,
		check_skin_appears,
		check_bullet_moves_forward,
		check_skin_disappears,
		check_bullet_visual_dies,
		check_bullet_dies_after_visual
	]
