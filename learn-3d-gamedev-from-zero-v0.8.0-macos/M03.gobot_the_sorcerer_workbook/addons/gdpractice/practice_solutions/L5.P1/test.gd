...
func _setup_state() -> void:
	...
	for p: Vector2 in [Vector2.ZERO, Vector2(size.x, 0.0), size, Vector2(0.0, size.y)]:
		...
		if _practice._ground_plane != null:
			test_data.expected_world_mouse_position = _practice._ground_plane.intersects_ray(_practice._camera_3d.global_position, test_data.expected_mouse_ray)
		else:
...
func _build_checks() -> void:
	...
	var check_ground_plane := Check.new()
	check_ground_plane.description = tr("The _ground_plane variable is a Plane object with a normal pointing up.")
	check_ground_plane.checker = func() -> String:
		if _practice._ground_plane == null:
			return tr("The _ground_plane variable has a value of null. Did you forget to assign a new Plane object to it?")
		elif not _practice._ground_plane.normal.is_equal_approx(Vector3.UP):
			return tr("The _ground_plane variable has a value of %s. For this practice, it should have a value of %s. Did you create a Plane object with a normal pointing up?" % [_practice._ground_plane, _solution._ground_plane])
		return ""
	...
	check_mouse_position.checker = func() -> String:
		...
		return ""
	check_mouse_position.dependencies = [check_ground_plane]
